...
- Every commit runs 1,000,000+ assertions across 107 non-exploit audit modules and 263 exploit PoCs (367 modules total; count via `python3 ci/sync_module_count.py`)
...
| Audit modules (`unified_audit_runner`) | **107 non-exploit modules + 263 exploit PoCs across 9 sections, 0 failures** |
...
- All 107 non-exploit audit modules and all 263 exploit PoCs return `AUDIT-READY` status as of the last CAAS gate run. Zero failures — see pinned evidence: [`docs/EXTERNAL_AUDIT_BUNDLE.json`](docs/EXTERNAL_AUDIT_BUNDLE.json).
...
The `unified_audit_runner` executes **107 non-exploit audit modules + 263 exploit PoCs** across 9 sections
...
| [Audit Coverage](docs/AUDIT_COVERAGE.md) | Full audit report with 107 non-exploit modules + 263 exploit PoCs and platform verdicts |
