...
- Every commit runs 1,000,000+ assertions across 99 non-exploit audit modules and 254 exploit PoCs (350 modules total; count via `python3 ci/sync_module_count.py`)
...
| Audit modules (`unified_audit_runner`) | **99 non-exploit modules + 254 exploit PoCs across 9 sections, 0 failures** |
...
- All 99 non-exploit audit modules and all 254 exploit PoCs return `AUDIT-READY` status as of the last CAAS gate run. Zero failures — see pinned evidence: [`docs/EXTERNAL_AUDIT_BUNDLE.json`](docs/EXTERNAL_AUDIT_BUNDLE.json).
...
The `unified_audit_runner` executes **99 non-exploit audit modules + 254 exploit PoCs** across 9 sections
...
| [Audit Coverage](docs/AUDIT_COVERAGE.md) | Full audit report with 99 non-exploit modules + 254 exploit PoCs and platform verdicts |
