...
> **ConnectBlock (primary block-validation workload):** ≈0% difference vs libsecp256k1 with Release+LTO build (`-DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON`). Without LTO (RelWithDebInfo): ~2.5% slower due to i-cache pressure. Taproot signing: 22–25% faster. Full numbers in [docs/BITCOIN_CORE_BENCH_RESULTS.json](docs/BITCOIN_CORE_BENCH_RESULTS.json).
...
- Every commit runs 1,000,000+ assertions across 103 non-exploit audit modules and 240 exploit PoCs
...
- All 91 non-exploit audit modules and all 252 exploit PoCs return `AUDIT-READY` status. Zero failures across all tested platforms.

### Exploit PoC Test Suite (252 Tests, 20+ Coverage Areas)

In addition to the 336-module `unified_audit_runner`, UltrafastSecp256k1 ships **252 registered exploit-style PoC test modules (254 test files)** that actively try to break the library across its highest-risk surfaces. Each `audit/test_exploit_*.cpp` target builds and runs standalone so failures stay easy to attribute and reproduce.
...
The `unified_audit_runner` executes **103 non-exploit audit modules + 240 exploit PoCs** across 9 sections
...
| [Audit Coverage](docs/AUDIT_COVERAGE.md) | Full audit report with 103 non-exploit modules + 240 exploit PoCs and platform verdicts |
