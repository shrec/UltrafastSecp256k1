...
- Every commit runs 1,000,000+ assertions across 107 non-exploit audit modules and 262 exploit PoCs (367 modules total; count via `python3 ci/sync_module_count.py`)
...
| Audit modules (`unified_audit_runner`) | **107 non-exploit modules + 262 exploit PoCs across 9 sections, 0 failures** |
...
- All 107 non-exploit audit modules and all 262 exploit PoCs return `AUDIT-READY` status as of the last CAAS gate run. Zero failures — see pinned evidence: [`docs/EXTERNAL_AUDIT_BUNDLE.json`](docs/EXTERNAL_AUDIT_BUNDLE.json).

### Exploit PoC Test Suite (262 Tests, 20+ Coverage Areas)

In addition to the 367-module `unified_audit_runner`, UltrafastSecp256k1 ships **262 exploit-style PoC modules files** that actively try to break the library across its highest-risk surfaces. Each `audit/test_exploit_*.cpp` target builds and runs standalone so failures stay easy to attribute and reproduce.
...
The `unified_audit_runner` executes **107 non-exploit audit modules + 262 exploit PoCs** across 9 sections
...
| [Audit Coverage](docs/AUDIT_COVERAGE.md) | Full audit report with 107 non-exploit modules + 262 exploit PoCs and platform verdicts |
