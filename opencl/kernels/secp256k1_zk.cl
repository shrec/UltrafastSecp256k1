...
// =============================================================================
// Uses RangeProofPolyGPU and range_proof_poly_check_impl defined above.
    ...
    const uint count)
{
    ...
    FieldElement out_x, out_y;
    field_mul_impl(&out_x, &result.x, &z_inv2);
    field_mul_impl(&out_y, &result.y, &z_inv3);
    commitments_out[gid].x = out_x;
    commitments_out[gid].y = out_y;
}
    ...
    __global int* result)
{
    ...
    for (uint i = 0; i < n_pos; ++i) {
        AffinePoint pos_pt = pos[i];
        point_add_mixed_impl(&sum, &sum, &pos_pt);
    }
