...
| surface | status |
|---------|--------|
| `ecdsa_verify` (single) | ✅ done + tested |
| `ecdsa_verify_batch` (multithreaded, fail-closed) | ✅ done + tested |
| `schnorr_verify` (single) | ✅ done + tested |
| `schnorr_verify_batch` (multithreaded, fail-closed) | ✅ done + tested |
| sign / recover / keys / math / serialize / context | ⏳ next increment (see below) |
| fine-grained batch cancellation hook | ⏳ TODO (coarse between-chunk cancel preserved) |
| silent payments (BIP-352 scan) | ⏳ planned (engine has `bip352_*`) |
| GPU verify / scan | ⏳ planned (engine GPU backends) |

Verify is the IBD-critical path. Test: `tests/test_direct_verify.cpp` (ECDSA +
Schnorr, single + batch, all-valid + fail-closed-on-tamper).
