...
-----
    ...
    python3 scripts/check_source_graph_quality.py --json
    python3 scripts/check_source_graph_quality.py --db /path/to/.project_graph.db
    python3 scripts/check_source_graph_quality.py --stale-hours 2
"""
...
def check_db_exists(db_path: Path) -> Check:
    ...
    return _fail(name, f"Database not found at {db_path}. "
                       f"Run: python3 scripts/build_project_graph.py --rebuild")
...
def check_graph_size(conn: sqlite3.Connection, tables: set[str]) -> Check:
    ...
    if total < TOTAL_ROW_FLOOR:
        ...
        return _fail(name,
            ...
            "Run: python3 scripts/build_project_graph.py --rebuild")
