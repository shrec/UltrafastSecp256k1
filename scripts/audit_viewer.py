...
Usage:
    python3 scripts/audit_viewer.py [--port 8765] [--host 127.0.0.1]
"""
...
def render_home() -> bytes:
    ...
    if kpi_path.exists():
        ...
        kpi_html = '<div class="alert">CAAS KPI not found — run <code>scripts/caas_runner.py</code> first.</div>'
    ...
    if bundle_path.exists():
        ...
        bundle_html = '<div class="alert">Evidence bundle not found — run <code>scripts/build_owner_audit_bundle.py</code>.</div>'
...
def render_bundle() -> bytes:
    ...
    if not bundle_path.exists():
        ...
        <div class="alert">
          ...
          <code>python3 scripts/build_owner_audit_bundle.py</code> to generate it.
        </div>"""
...
def render_graph() -> bytes:
    if not GRAPH_DB.exists():
        ...
        <div class="alert">
          ...
          Run <code>python3 scripts/build_project_graph.py</code> to build it.
        </div>"""
