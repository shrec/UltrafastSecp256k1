...
Usage:
  ...
  python3 scripts/audit_gap_report.py --json
  python3 scripts/audit_gap_report.py --strict
  python3 scripts/audit_gap_report.py --json -o audit_gap_report.json
"""
...
def _looks_like_repo_path(token: str) -> bool:
    ...
    if re.search(r'\.[A-Za-z0-9_+-]+$', normalized):
        return True
    if normalized.startswith(('docs/', 'audit/', 'scripts/', 'tools/', 'include/', 'cpu/', 'gpu/', 'opencl/', 'metal/')):
        return True
    return False
