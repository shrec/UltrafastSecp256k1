...
# ============================================================================
# Usage:
#   ./scripts/ci_local.sh           # quick gates only (~30s)
#   ./scripts/ci_local.sh --full    # quick + build + tests (~10min)
#   ./scripts/ci_local.sh --msan    # also runs no-ASM build (MSan path)
#
# Install as pre-push hook:
#   ln -sf ../../scripts/ci_local.sh .git/hooks/pre-push
# ============================================================================
set -uo pipefail
...
run_check "Exploit wiring parity"   python3 scripts/check_exploit_wiring.py
run_check "Canonical data sync"     python3 scripts/build_canonical_data.py --dry-run
run_check "Docs from canonical"     python3 scripts/sync_docs_from_canonical.py --dry-run
run_check "Module count sync"       python3 scripts/sync_module_count.py --dry-run
echo ""

# ── Code quality scanners (~15s) ─────────────────────────────────────────────
echo -e "${BOLD}[2] Code Quality Scanners${NC}"
run_check "hot_path_alloc regression"  python3 scripts/run_code_quality.py --fail-on-regression --json > /dev/null
...
run_check "Preflight --bug-scan"    python3 scripts/preflight.py --bug-scan
echo ""
