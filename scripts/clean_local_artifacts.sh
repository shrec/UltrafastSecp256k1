...
#
# Usage:
#   bash scripts/clean_local_artifacts.sh          # dry-run (shows what would be removed)
#   bash scripts/clean_local_artifacts.sh --delete  # actually delete
...
for d in build build-* build_*; do
    # Skip scripts/files that start with "build" but are not directories
    [[ -d "$d" ]] && _remove "$d"
done
