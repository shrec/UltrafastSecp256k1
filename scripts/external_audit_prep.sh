...
Usage:
  bash scripts/external_audit_prep.sh [options]
...
if [[ "$SKIP_GRAPH" -eq 0 ]]; then
    run_step "build project graph" python3 "$PROJECT_ROOT/scripts/build_project_graph.py" --rebuild
fi

run_step "preflight security" python3 "$PROJECT_ROOT/scripts/preflight.py" --security
run_step "preflight abi" python3 "$PROJECT_ROOT/scripts/preflight.py" --abi
run_step "preflight freshness" python3 "$PROJECT_ROOT/scripts/preflight.py" --freshness
run_step "preflight drift" python3 "$PROJECT_ROOT/scripts/preflight.py" --drift
run_step "preflight coverage advisory" bash -lc "python3 '$PROJECT_ROOT/scripts/preflight.py' --coverage || true"
run_step "preflight changed advisory" bash -lc "python3 '$PROJECT_ROOT/scripts/preflight.py' --changed || true"

run_step "validate assurance" bash -lc "python3 '$PROJECT_ROOT/scripts/validate_assurance.py' --json | tee '$OUTPUT_DIR/validate_assurance.json'"
run_step "export assurance" python3 "$PROJECT_ROOT/scripts/export_assurance.py" -o "$OUTPUT_DIR/assurance_report.json"
cp "$PROJECT_ROOT/docs/ASSURANCE_CLAIMS.json" "$OUTPUT_DIR/assurance_claims.json"

if [[ "$SKIP_TRACEABILITY" -eq 0 ]]; then
    run_step "generate traceability" bash "$PROJECT_ROOT/scripts/generate_traceability.sh" "$PROJECT_ROOT/$TRACEABILITY_BUILD_DIR"
...
if [[ "$WITH_PACKAGE" -eq 1 ]]; then
    ...
    run_step "generate audit package" bash "$PROJECT_ROOT/scripts/generate_audit_package.sh" --build-dir "$BUILD_DIR"
