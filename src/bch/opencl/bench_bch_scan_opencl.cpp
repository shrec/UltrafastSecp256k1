...
// Must match BCHScanKeyGlv typedef in secp256k1_bch_scan.cl exactly.
struct BCHScanKeyGlv {
...
std::string load_bch_scan_kernel_source() {
    ...
    src += "\n";
    // BCH scan kernel (no #include — deps already loaded above)
    src += expand_kernel_file(std::string(SECP256K1_OPENCL_KERNEL_DIR) + "/../../bch/opencl/secp256k1_bch_scan.cl", stack);
    return src;
}
...
BCHScanKeyGlv build_scan_glv_plan() {
    BCHScanKeyGlv out{};
...
int main(int argc, char** argv) {
    ...
    std::cout << "============================================================\n";
    std::cout << "  BCH RPA Scanner Pipeline: CPU vs OpenCL\n";
    std::cout << "============================================================\n";
    ...
    std::cout << "Building OpenCL BCH pipeline kernel...\n";
    std::string source = load_bch_scan_kernel_source();
    ...
    const char* kernel_name = use_lut ? "bch_scan_kernel_lut" : "bch_scan_kernel_glv";
    ...
    BCHScanKeyGlv scan_plan{};

    // Both paths now use BCHScanKeyGlv with precomputed wNAF digits.
    ...
    cl_mem d_scan = clCreateBuffer(cl_ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                   sizeof(BCHScanKeyGlv), &scan_plan, &err);
    ...
    auto tagged = secp256k1::detail::cached_tagged_hash(
        secp256k1::detail::make_tag_midstate("BCH/SharedSecret"),
        shared_ser,
        sizeof(shared_ser));
