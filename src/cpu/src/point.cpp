...
// ESP32/bare-metal: these FieldElement global ctors run before FreeRTOS starts
// (coexist ROM timer fires during init → crash). Use lazy init on ESP32.
#if defined(ESP_PLATFORM) || defined(SECP256K1_PLATFORM_ESP32)
static inline const FieldElement& gen_gx() {
...
static inline const FieldElement& gen_gy() {
    static FieldElement v = FieldElement::from_bytes({
...
static inline const FieldElement& gen_negy() {
    static FieldElement v = FieldElement::from_bytes({
...
#  define kGeneratorX    gen_gx()
#  define kGeneratorY    gen_gy()
#  define kNegGeneratorY gen_negy()
#else
