...
// Must match LTCSPScanKeyGlv typedef in secp256k1_ltcsp_scan.cl exactly.
...
std::string load_ltcsp_kernel_source() {
    // Load dependency chain from opencl/kernels/ then ltcsp kernel from ltc/opencl/
    std::set<std::string> stack;
    std::string src = expand_kernel_file(std::string(SECP256K1_OPENCL_KERNEL_DIR) + "/secp256k1_extended.cl", stack);
    src += "\n";
    // hash160 for sha256_oneshot_impl (double_sha256_64 in ltcsp kernel)
    src += expand_kernel_file(std::string(SECP256K1_OPENCL_KERNEL_DIR) + "/secp256k1_hash160.cl", stack);
    src += "\n";
    // LTCSP scan kernel (no #include — deps already loaded above)
    src += expand_kernel_file(std::string(SECP256K1_OPENCL_KERNEL_DIR) + "/../../ltc/opencl/secp256k1_ltcsp_scan.cl", stack);
    return src;
}
...
LTCSPScanKeyGlv build_scan_glv_plan() {
    LTCSPScanKeyGlv out{};
...
int main(int argc, char** argv) {
    ...
    std::cout << "============================================================\n";
    std::cout << "  LTC-SP Silent Payments Pipeline: CPU vs OpenCL\n";
    std::cout << "============================================================\n";
    ...
    std::cout << "Building OpenCL LTCSP pipeline kernel...\n";
    std::string source = load_ltcsp_kernel_source();
    ...
    const char* kernel_name = use_lut ? "ltcsp_pipeline_kernel_lut" : "ltcsp_pipeline_kernel";
    ...
    LTCSPScanKeyGlv scan_plan{};

    // Both paths now use LTCSPScanKeyGlv with precomputed wNAF digits.
    ...
    cl_mem d_scan = clCreateBuffer(cl_ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                   sizeof(LTCSPScanKeyGlv), &scan_plan, &err);
    ...
    auto tagged = secp256k1::detail::cached_tagged_hash(
        secp256k1::detail::make_tag_midstate("LTCSP/SharedSecret"),
        shared_ser,
        sizeof(shared_ser));
