    ...
    "UltrafastSecp256k1 Performance": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "d922d65da000ec31a37180f19dad5863d9e79b8b",
          "message": "security: fix 20 audit findings — GPU/ABI/shim/CAAS hardening\n\nCRITICAL\n- Metal schnorr_sign_batch: replaced private key as aux_rand with zero bytes (BIP-340 violation)\n- Metal ecdsa/schnorr_sign_batch: route through ct_ecdsa_sign_metal/ct_schnorr_sign_metal;\n  check return value; zero output on failure (fail-open → fail-closed)\n\nHIGH\n- Metal ECDH: zero private key from device buffer + thread-local scratch after use\n- CUDA bip352_scan_batch: cudaMemset(d_scan_k) + secure_erase(h_scan_k) before free\n- OpenCL bip352_scan_batch: clEnqueueFillBuffer(d_plan) + secure_erase GLV sub-scalars\n\nMEDIUM\n- Schnorr ABI (native + shim): check R x-coord all-zero in addition to s==0\n- MuSig2 partial_sign: new ct::scalar_mul implementation; partial sign now CT\n- OpenCL/Metal non-CT schnorr_sign: add s==0 check before returning success\n- GPU non-CT Schnorr: replace branchy parity flip with branchless scalar_cmov\n- advisory_skipped heuristic: replace elapsed_ms<1.0 with ADVISORY_SKIP_CODE=77 sentinel\n\nLOW/OTHER\n- Batch sign APIs reject count==0 with UFSECP_ERR_BAD_INPUT\n- CUDA batch sign: zero-init signature struct before call\n- check_exploit_wiring.py: now also verifies audit/CMakeLists.txt listing\n\nLegacy C API / bindings\n- All private key inputs use parse_bytes_strict_nonzero (rejects >=n and 0)\n- CT pubkey derivation: ct::generator_mul replaces Point::generator().scalar_mul\n- Route signing through secp256k1::ct::ecdsa_sign/schnorr_sign\n- Degenerate output checks (is_valid() + all-zero r/s) with output zeroing\n\nMuSig2 shim\n- find_index() returns {idx,bool}; unknown signer fails, not silently signs as index 0\n- CT pubkey derivation in signer lookup (ct::generator_mul_blinded)\n\nBCHN Schnorr shim\n- Strict key parsing (parse_bytes_strict_nonzero)\n- CT generator_mul_blinded for nonce R and pubkey P\n- secure_erase for key material on all exit paths\n\nlibsecp256k1 shim\n- Context flag enforcement: SIGN context required for sign, VERIFY for verify\n- secp256k1_schnorrsig_sign_custom: add header declaration + DIVERGENCE comments\n\nCAAS/CI\n- audit_gate.py: fix source_graph.db vs .project_graph.db naming (G-12)\n- caas.yml: check_bitcoin_core_test_results.py added as hard gate\n- caas.yml: audit_gate.py exit code propagation hardened\n- check_bitcoin_core_test_results.py: backend_commit freshness check\n\nNew CAAS tests (9 files, 246 total → all wired)\n- test_exploit_legacy_capi_key_parsing\n- test_exploit_legacy_capi_degenerate_sig\n- test_exploit_musig_unknown_signer\n- test_exploit_bchn_schnorr_strict_parsing\n- test_exploit_context_flag_bypass\n- test_exploit_metal_schnorr_aux_rand\n- test_exploit_metal_batch_failclosed\n- test_exploit_gpu_bip352_key_erase\n- test_exploit_metal_ecdh_key_erase\n\nDocs: AUDIT_CHANGELOG, API_REFERENCE, BITCOIN_CORE_BACKEND_EVIDENCE updated\nCLAUDE.md: GPU + Binding/Shim Security Guardrails rules 8–16 added\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>",
          "timestamp": "2026-05-01T18:08:47Z",
          "tree_id": "30837ec4b67f24e790f4fe52b82545f392014736",
          "url": "https://github.com/shrec/UltrafastSecp256k1/commit/d922d65da000ec31a37180f19dad5863d9e79b8b"
        },
        "date": 1777660909452,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "4e7f6da4c300136c67522cb26b72bc05ab676b5a",
          "message": "hardening: WS1-WS8-WS12 — product taxonomy, CAAS profiles, CT claims corrected\n\nWS1 — Product/Profile Taxonomy\n- docs/PRODUCT_PROFILES.md: new file with 7 profiles, 5 tiers, 10 surface entries,\n  claims wording policy (what is/isn't claimed CT-safe)\n- docs/API_SECURITY_CONTRACTS.json: v1.1.0 — updated last_updated + audit note\n\nWS7 — Public C++ Signing CT Claims Corrected\n- cpu/include/secp256k1/ecdsa.hpp: FIXED incorrect \"FAST, non-constant-time\" comment\n  (secp256k1::ecdsa_sign uses ct::generator_mul_blinded internally — IS CT)\n- cpu/include/secp256k1/schnorr.hpp, recovery.hpp, ct/sign.hpp: CT doc annotations\n- Added SECP256K1_SIGNING_IS_CT sentinel macro\n- docs/SECURITY_CLAIMS.md: corrected erroneous \"variable-time\" entry for\n  secp256k1::ecdsa_sign/schnorr_sign; added dated correction note\n\nWS8 — CAAS Profile System\n- scripts/caas_runner.py: added ffi-bindings, wasm, bchn-compat profiles\n  (all 3 AUDIT PASS: ffi-bindings 100/100, wasm PASS, bchn-compat PASS)\n- scripts/external_audit_bundle.py: remove SECURITY_AUTONOMY_KPI.json from\n  EVIDENCE_FILES (fixes hash-race: auto-generated file, score captured in gate_results)\n\nWS12 — Documentation\n- docs/CAAS_PROTOCOL.md: added Product Profiles section with runner commands\n- docs/BACKEND_ASSURANCE_MATRIX.md: added non-GPU profile assurance table\n  (bitcoin-core-backend, cpu-signing, ffi-bindings, wasm, bchn-compat)\n\nEvidence freshness\n- audit/ci-evidence/*_20260501.txt: new CI evidence files (reset 14-day SLA)\n\nAll CAAS profiles passing:\n  bitcoin-core-backend  AUDIT PASS  score=100/100  5/5 stages\n  cpu-signing           AUDIT PASS  score=100/100\n  ffi-bindings          AUDIT PASS  score=100/100\n  wasm                  AUDIT PASS\n  gpu-public-data       AUDIT PASS\n  bchn-compat           AUDIT PASS\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>",
          "timestamp": "2026-05-01T18:43:53Z",
          "tree_id": "78fd7dab6a95d9e7cf080a68ec09e2b2192fe8d6",
          "url": "https://github.com/shrec/UltrafastSecp256k1/commit/4e7f6da4c300136c67522cb26b72bc05ab676b5a"
        },
        "date": 1777662973156,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "d0c73925be3719710434ce119be2fa0c17de6d6a",
          "message": "fix(ci): fix 3 CI failures — GPU standalone linker, bundle digest, ccache hash\n\n1. audit/CMakeLists.txt: wrap test_regression_gpu_key_erase_raii_standalone\n   in if(TARGET secp256k1_gpu_host OR ...) guard. The source includes\n   ufsecp_gpu.h and calls ufsecp_gpu_ctx_create/ecdh_batch/destroy — without\n   the GPU host library these symbols are undefined at link time, breaking\n   ASan/MSan/TSan/Valgrind builds on CPU-only CI runners.\n\n2. docs/EXTERNAL_AUDIT_BUNDLE.{json,sha256}: update evidence hashes for\n   docs/TEST_MATRIX.md and docs/CT_VERIFICATION.md. Both files were updated\n   in the recent perf commits but the bundle was not regenerated, causing\n   verify_external_audit_bundle.py to report digest mismatch → CAAS gate\n   failure on every push.\n\n3. .github/workflows/bench-regression.yml: fix ccache-action pinned hash.\n   a1209f81... was not a valid commit in the hendrikmuhs/ccache-action repo;\n   replaced with 5ebbd400... which is the actual commit behind the v1.2 tag.\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>",
          "timestamp": "2026-05-05T15:23:48Z",
          "tree_id": "ef8e7f39a5e94916b5d681d3cafe105a619e2baf",
          "url": "https://github.com/shrec/UltrafastSecp256k1/commit/d0c73925be3719710434ce119be2fa0c17de6d6a"
        },
        "date": 1777996852957,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "913b59e728eb19710778503ad1c5081cbeae298d",
          "message": "fix(build): enforce out/ convention; guard against stray root build dirs\n\nProblem: cmake -B build examples in docs caused agents and users to create\nbuild/, build_bench_opt/, build-ci/, build-quick/ at the repo root instead of\nunder out/ where .gitignore and all tooling expects them.\n\nChanges:\n- CMakeLists.txt: fatal error on in-source builds; warning if binary dir is\n  inside source tree but not under out/\n- scripts/preflight.py: check_ctest_registry_health() now scans out/ subdirs\n  instead of legacy root-level build* paths\n- scripts/create_replay_capsule.py: replay commands use out/release\n- docs/BUILDING.md, README.md, CONTRIBUTING.md, USER_GUIDE.md,\n  CT_EMPIRICAL_REPORT.md, AUDIT_READINESS_REPORT_v1.md,\n  AUDIT_TRACEABILITY.md, CAAS_THREAT_MODEL.md, bench_nodes/README.md,\n  cmake/toolchain-*.cmake, include/ufsecp/CMakeLists.txt:\n  all cmake -S . -B build* examples → out/<profile>\n\nNote: build-ci/ (root) owned by root — delete manually with sudo rm -rf build-ci\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>",
          "timestamp": "2026-05-05T16:32:50Z",
          "tree_id": "28210cf4f18915f633e36a95bb47bf22d4c45619",
          "url": "https://github.com/shrec/UltrafastSecp256k1/commit/913b59e728eb19710778503ad1c5081cbeae298d"
        },
        "date": 1778001310803,
        "tool": "customSmallerIsBetter",
        "benches": [
