...
// Include the unchanged P6 driver so its actual measure(), main(), and JSON
...
#define main p6_driver_main
#include "../probes/p6_field_boundary_compare.cpp"
#undef main
...
unsigned test_measure(const Corpus& corpus, const std::vector<Cell>& cells) {
    unsigned cases = 0;
    require(cells.size() == 16, "P6 matrix must contain all 16 cells");
    for (const auto& cell : cells) {
        require(cell.validated, "direct measure cell was not validated");
        require(cell.lane0_validated, "direct measure cell lacks cross-mode lane-zero agreement");
...
std::string test_main_failure(const std::vector<Cell>& expected_cells) {
    char directory[] = "/tmp/parseatlas-p6-duration.XXXXXX";
    ...
    std::array<std::string, 4> args{"p6_synthetic_duration_test", "--smoke", "--output", output};
    ...
    // clock call. Exactly 1ms qualifies each at one job. After all 16 cells,
    ...
    int result = -1;
    {
        ...
        result = p6_driver_main(static_cast<int>(argv.size()), argv.data());
    ...
    for (const auto& item : cells) {
        ...
        require(cell.get<bool>("lane0_validated"),
                "serialized cell lacks cross-mode lane-zero agreement");
...
int main() {
    try {
        if (!pa_p6::native_asm_available())
            throw std::runtime_error("P6 duration diagnostic requires the native ASM routes");
