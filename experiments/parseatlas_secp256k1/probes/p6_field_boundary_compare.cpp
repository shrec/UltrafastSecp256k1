// P6 field arithmetic: API/ASM/inline/outlined boundary controls; research only, no LTO.
...
#else
#error "P6 requires GCC or Clang compiler memory barriers"
#endif
...
using Wide = std::array<std::uint64_t, 8>; // P5 corpus control bytes, unused by P6 jobs.
using States = std::array<Limbs, 4>;
using Bytes = std::array<std::uint8_t, 128>;
using Clock = std::chrono::steady_clock;
constexpr unsigned steps = 1024, corpus_count = 256, cell_count = 16;
constexpr std::array<const char*, 8> routes{
    "mul_api", "mul_asm_direct", "mul_row_inline", "mul_row_outlined",
    "square_api", "square_asm_direct", "square_row_inline", "square_row_outlined"};
constexpr std::array<const char*, 2> operations{"multiply", "square"};
constexpr std::array<unsigned, 4> group_start{0,4,8,12};
...
template<unsigned Route> inline Limbs primitive(const Limbs& x, const Corpus& corpus, unsigned step) {
    ...
    else if constexpr (Route == 1) return pa_p6::asm_mul_direct(x, corpus.rhs[step & (corpus_count - 1)].limbs());
    else if constexpr (Route == 2) return pa_p6::row_mul_inline(x, corpus.rhs[step & (corpus_count - 1)].limbs());
    else if constexpr (Route == 3) return pa_p6::row_mul_outlined(x, corpus.rhs[step & (corpus_count - 1)].limbs());
    else if constexpr (Route == 4) return FE::from_limbs_raw(x).square().limbs();
    else if constexpr (Route == 5) return pa_p6::asm_square_direct(x);
    else if constexpr (Route == 6) return pa_p6::row_square_inline(x);
    else return pa_p6::row_square_outlined(x);
}
...
constexpr std::array<Stepper,8> steppers{
    primitive<0>,primitive<1>,primitive<2>,primitive<3>,
    primitive<4>,primitive<5>,primitive<6>,primitive<7>};
...
struct Cell {
    ...
    unsigned lanes;
    Bytes expected{};
    bool validated = false, qualified = false, lane0_validated = false;
...
std::vector<Cell> make_cells() {
    ...
    for (unsigned group = 0; group < 4; ++group) {
        ...
        const unsigned first_route = group / 2 * 4, width = 4;
...
Limbs reference_step(const Limbs& x, const Corpus& corpus, unsigned step, unsigned kind) {
    ...
    return FE::from_limbs_raw(x).square().limbs();
}
void validate(std::vector<Cell>& cells, const Corpus& corpus) {
    ...
    for (unsigned group = 0; group < 4; group += 2) {
        for (unsigned offset = 0; offset < 4; ++offset) {
            auto& chain = cells[group_start[group] + offset];
            auto& ilp = cells[group_start[group + 1] + offset];
            if (!std::equal(chain.expected.begin(), chain.expected.begin() + 32, ilp.expected.begin()))
                throw std::runtime_error("lane0 cross-mode mismatch");
            chain.lane0_validated = ilp.lane0_validated = true;
        }
    }
}
...
struct Order { std::array<unsigned,4> groups{0,1,2,3}; std::array<unsigned,4> routes{0,1,2,3}; };
Order base_order(std::uint64_t seed) {
    Order order; RNG rng{seed ^ UINT64_C(0x6f726465725f7036)};
...
std::vector<unsigned> schedule(const Order& base, unsigned round, bool reverse) {
    ...
    for (const auto group : groups) {
        ...
        for (const auto offset : variants)
            order.push_back(group_start[group]+offset);
    }
...
std::string json(const Options& options, const Corpus& corpus, const std::vector<Cell>& cells,
    ...
    out << "{\n\"schema\":\"parseatlas.p6.field_boundary_compare.v1\",\n\"status\":" << quote(status)
    ...
    out << ",\n\"contract\":{"
        "\"scope\":\"unchanged canonical field multiply/square arithmetic under API/directASM/inlineRow/separateTUrow boundaries; no production, CT, novelty or upper-layer claim\","
        ...
        "\"native_admission\":\"caller checks Linux x86-64 ASM/BMI2/ADX support before any trace or job; native direct wrappers have no added per-call dispatch\","
        "\"boundaries\":\"API retains production returning operators; directASM and outlinedRow call external pointer/output leaves; inlineRow uses unchanged P5 arithmetic; separateTU and fno-lto preserve row boundary\","
        ...
        "\"ordering\":\"seeded4group/4route-slot permutation plus round rotation/reversal; second series reverses complete order\","
        ...
        "\"backend\":\"external manifest binds exact boundaryheader/separateTU/driver/P5kernel/library/binary/compiler flags; harness macros alone do not prove library configuration\","
        ...
        << ",\"recipe\":\"Unchanged P5 corpus identity, raw512 control bytes unused by P6 jobs: SplitMix64 state=seed xor0x6669656c645f7035; canonical candidates four RNGwords encoded BE32, strict reject>=p or zero; generate4seeds then256RHS; then256rawWide each8RNGwords in little-limb order\","
        ...
        << ",\"raw512_count\":" << corpus_count << ",\"raw512_used_by_jobs\":false,\"seed_bytes\":" << sizeof(corpus.seeds)
    ...
    for (const auto& cell:cells) {
        ...
        out << "{\"id\":" << cell.id << ",\"group\":" << cell.group << ",\"route_index\":" << cell.route
            ...
            << ",\"lane0_validated\":" << cell.lane0_validated
    ...
    out << "\n],\n\"primary_comparisons\":[\n";
    bool first=true;
    for(unsigned g=0;g<4;++g) for(unsigned offset=1;offset<4;++offset) {
        if(!first)out<<",\n";
        first=false;
        write_comparison(out,cells,group_start[g],group_start[g]+offset,options,records);
    }
    out << "\n],\n\"secondary_comparisons\":[\n";first=true;
    constexpr std::array<std::pair<unsigned,unsigned>,3> boundary_pairs{{{1,2},{1,3},{2,3}}};
    for(unsigned g=0;g<4;++g) for(const auto& [a,b]:boundary_pairs) {
        if(!first)out<<",\n";
        first=false;
        write_comparison(out,cells,group_start[g]+a,group_start[g]+b,options,records);
    }
    out << "\n],\n\"mode_comparisons\":[\n";first=true;
    for(unsigned g=0;g<4;g+=2) for(unsigned offset=0;offset<4;++offset) {
        if(!first)out<<",\n";
        first=false;
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (options.help) {
            std::cout << "Usage: p6_field_boundary_compare --output NEW.json [--seed UINT64] [--min-ms 0.1..1000] [--rounds EVEN_2..128] [--reverse-order] [--smoke]\n"
                "16cells: API/directASM/inlineRow/outlinedRow x mul/square x chain1/ILP4. Default200ms, two warmups/eight measured rounds.\n"
        ...
        try {
            if (!pa_p6::native_asm_available()) throw std::runtime_error("native BMI2/ADX ASM unavailable; no P6 jobs executed");
        ...
        std::cout << "P6 " << status << " cells=" << cells.size() << " raw_regions=" << records.size() << " output=" << options.output << '\n';
        if (!failure.empty()) std::cerr << failure << '\n';
        return result;
    } catch (const std::exception& error) {
        if (fd >= 0) ::close(fd);
        std::cerr << "P6 error: " << error.what() << '\n'; return 1;
    }
}
