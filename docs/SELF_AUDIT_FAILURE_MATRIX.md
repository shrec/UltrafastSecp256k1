...
| Formal invariant compliance | Missing or incomplete formal invariant specs for critical operations | `docs/FORMAL_INVARIANTS_SPEC.json`, `scripts/check_formal_invariants.py`, `docs/INVARIANTS.md`, `docs/CRYPTO_INVARIANTS.md` | Covered | Automated via `check_formal_invariants.py`; 7 critical operations with preconditions, postconditions, and CT linkage validated |
| Audit SLA / evidence staleness | Evidence drifts past age limits without detection, stale golden references | `docs/AUDIT_SLA.json`, `scripts/audit_sla_check.py`, `docs/SECURITY_AUTONOMY_KPI.json`, `docs/DETERMINISM_GOLDEN.json` | Covered | SLA enforcement infra complete; `DETERMINISM_GOLDEN.json` generated and validated; `audit_sla_check.py` returns `release_ready: True`; `check_determinism_gate.py` returns `overall_pass: True` |
| Incident response readiness | Untested triage and disclosure paths | `docs/INCIDENT_RESPONSE.md`, `scripts/incident_drills.py` | Covered | Three automated drills (key compromise, CI poisoning, dependency compromise) run as preflight step 20/20 |
