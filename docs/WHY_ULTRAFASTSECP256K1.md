...
| Exploit PoC tests | 272 dedicated adversarial PoC modules across 20+ coverage categories (`audit/test_exploit_*.cpp`) | 272 wired, 0 failures |
...
| **Total (audit runner)** | **unified_audit_runner** across 173 non-exploit modules + 272 exploit-PoC modules (445 total) | **~1,000,000+** |
...
All 173 non-exploit audit modules across all tested platforms return **AUDIT-READY**. Zero failures in the current CI run (not a lifetime claim — see `docs/AUDIT_COVERAGE.md` for current status).
