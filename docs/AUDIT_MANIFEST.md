...
| 2026-06-19 | Added **G-21 Libbitcoin Performance Matrix** (Bastion B21): `ci/check_libbitcoin_perf_matrix.py` (`audit_gate.py --libbitcoin-perf-matrix`, in `ALL_CHECKS`) validates `docs/LIBBITCOIN_PERF_MATRIX_STATUS.json` — libbitcoin integration/performance rows must declare `target_context=libbitcoin`, evidence paths, reproduce commands, claim scope, copy policy, native-hardware scope, and whether a structured benchmark artifact contract exists. Negative fixture `B21:libbitcoin_perf_matrix` proves fail-closed on missing required surfaces, wrong context, missing evidence, native overclaim, and missing JSON benchmark artifact contract. | Libbitcoin acceleration claims are now machine-gated: no row/column/no-copy/MSVC/GPU benchmark claim can be treated as evidence without an explicit context, evidence path, reproduce command, and copy/native-hardware policy |
