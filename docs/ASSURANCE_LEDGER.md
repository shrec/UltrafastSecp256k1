...
| A-003 | GPU parity | CUDA, OpenCL, and Metal implement the stable public GPU surface | Public 19-op GPU ABI; not a statement about every experimental kernel | `include/ufsecp/ufsecp_gpu.h`, `docs/BACKEND_ASSURANCE_MATRIX.md`, backend adapters | GPU backend matrix tests, backend audit runners | Backend audit logs | Per GPU-enabled validation run | Active | Medium | `src/gpu/src/*`, `metal/*`, `opencl/*`, `docs/BACKEND_ASSURANCE_MATRIX.md` |
