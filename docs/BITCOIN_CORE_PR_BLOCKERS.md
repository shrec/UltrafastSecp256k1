...
| 7 | `VERSION.txt` stale | ✅ **DONE** | `VERSION.txt` = `4.1.1` (matches release tag) |
...
|---|------|--------|---------|
| 10 | Bitcoin Core workload benchmarks | ✅ **DONE** | `docs/BITCOIN_CORE_BENCH_RESULTS.json` (2026-05-12, hard turbo lock) — **With Release+LTO (GCC 14, required for any positive result):** ConnectBlock: **+0.9–+1.5% faster across all profiles (AllEcdsa, AllSchnorr, Mixed)** — 5 runs, err% 0.1–0.5%. **Without LTO: ~0.5–1.0% SLOWER (regression — LTO required).** Taproot signing (SignSchnorrWithMerkleRoot): **+35% faster (1.35×)**; P2TR script-path verification is ~10% faster (1.10×)**; SignTransactionECDSA: **+10% faster (1.10×)**; SignTransactionSchnorr: **+10% faster (1.10×)**. **Without LTO: ~0.5–1.0% SLOWER** (i-cache pressure from larger footprint; PERF-002 reduced gap from ~1.1% but LTO is required for Ultra to win). See `docs/BITCOIN_CORE_BACKEND_EVIDENCE.md §2.1`. |
...
|----------|--------|---------|
| **Code Quality** | ✅ A | CAAS 275 exploit PoCs, 0 drift |
...
|------|--------|---------------|
| Stage 0 — exploit wiring | ✅ 334/334 | Every `test_exploit_*.cpp` (261) and `test_regression_*.cpp` (73) has `_run()` in runner |
...
*Last updated: 2026-05-21 — Security fixes: CT-001 ellswift non-CT gen_mul, CT-003 schnorr_xonly branchless parity, RT-004 HMAC pad erase, CT-006 MuSig2 k1/k2 strict parsing, RT-011 BIP32 private_key() strict, SHIM-002 context_create callback, SHIM-012 pubkey_sort crash. Stage 0 updated 235→254. Stage 2e updated 693/693 (Clang 19) → 749/749 (GCC 14, 2026-05-20). ConnectBlock: Ultra is +0.9–1.5% faster than libsecp256k1 with Release+LTO (confirmed, err% 0.2–0.5%, hard turbo lock 2026-05-12) and ~0.5–1.0% slower WITHOUT LTO — LTO is required to win all ConnectBlock workloads (see docs/BITCOIN_CORE_BENCH_RESULTS.json and canonical_numbers.json `connectblock`). All blockers remain closed.*
